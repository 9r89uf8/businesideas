...
export async function deliverAwsCollectorResult({
  ...
  const result = normalizeCollectorResult(outcome);
  if (
...
export async function runAwsCollectorCommand({
  ...
  try {
    ...
    const resultUrl = mode === COLLECTOR_COMMAND_MODES.COLLECT
    ...
    if (mode === COLLECTOR_COMMAND_MODES.COLLECT && result.status === "completed") {
      await deliverResult({
        resultUrl,
    ...
    log(
      error?.code === X_FOR_YOU_ERROR_CODES.FEATURE_DISABLED
        ? "PERMISSION_DENIED"
        : "RUN_FAILED",
      fields,
    );
    return Object.freeze({
      exitCode: error instanceof XForYouSafetyError ? 2 : 1,
      status: "failed",
