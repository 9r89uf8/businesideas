  ...
  return (
    <header className="border-b border-[var(--line)] bg-[rgb(251_250_246/0.86)] backdrop-blur-xl">
      <div className="shell flex min-h-18 items-center justify-between gap-5 py-3">
        ...
        <nav aria-label="Primary navigation" className="flex items-center gap-1 rounded-xl border border-[var(--line)] bg-white/60 p-1">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              aria-current={isCurrent(pathname, link.href) ? "page" : undefined}
              className={`focus-ring rounded-lg px-3 py-2 text-xs font-semibold transition-colors sm:text-sm ${
        ...
        <button
          ...
          onClick={signOut}
          disabled={signingOut}
          className="focus-ring hidden rounded-lg px-2 py-2 text-xs font-semibold text-[var(--ink-soft)] transition-colors hover:bg-white hover:text-[var(--ink)] disabled:opacity-50 sm:block"
        >
