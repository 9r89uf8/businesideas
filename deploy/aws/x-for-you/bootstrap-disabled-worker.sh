...
usage() {
  printf '%s\n' \
    "Usage: sudo ./deploy/aws/x-for-you/bootstrap-disabled-worker.sh --secret-id ID --region REGION [--repository /opt/signal-foundry] [--runtime PATH] [--post-limit N]"
}
...
if [[ "${EUID}" -ne 0 ]] || [[ "$(uname -s)" != "Linux" ]]; then
  printf '%s\n' "The disabled AWS worker bootstrap must run as root on Linux." >&2
  exit 1
fi
...
if (
  ...
  [[ ! -f "${repository_directory}/scripts/run-x-for-you-aws.js" ]] ||
  ...
  printf '%s\n' "The disabled AWS worker bootstrap arguments are invalid." >&2
  exit 1
fi
...
if (
  ...
  printf '%s\n' "The disabled AWS worker runtime path is unsafe." >&2
  exit 1
fi
...
if ! /usr/sbin/runuser -u "${worker_user}" -- \
  /usr/bin/test -r "${repository_directory}/scripts/run-x-for-you-aws.js"; then
  printf '%s\n' "The AWS X collector bundle is not readable by the worker." >&2
  exit 1
fi
...
printf '%s\n' "X_FOR_YOU_AWS_WORKER_CONFIGURED_DISABLED"
printf '%s\n' "No authorization values or X credentials were installed."
