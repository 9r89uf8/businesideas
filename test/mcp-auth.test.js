...
test("OAuth consent accepts only the supported identity scopes and safe redirects", () => {
  ...
  assert.equal(
    normalizeOAuthRedirectUrl(
      "http://127.0.0.1:57098/callback/native?code=one&state=two",
      "http://127.0.0.1:57098/callback/native",
    ),
    "http://127.0.0.1:57098/callback/native?code=one&state=two",
  );
  assert.equal(
    normalizeOAuthRedirectUrl(
      "http://[::1]:57098/callback/native?code=one&state=two",
      "http://[::1]:57098/callback/native",
    ),
    "http://[::1]:57098/callback/native?code=one&state=two",
  );
  assert.equal(
    normalizeOAuthRedirectUrl(
      "http://127.0.0.1:57099/callback/native?code=one",
      "http://127.0.0.1:57098/callback/native",
    ),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl(
      "http://127.0.0.1:57098/callback/other?code=one",
      "http://127.0.0.1:57098/callback/native",
    ),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl("http://localhost:57098/callback/native"),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl("http://127.0.0.1/callback/native"),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl("http://signal-foundry.example/callback"),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl("http://127.0.0.2:57098/callback/native"),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl(
      "http://127.0.0.1.evil.example:57098/callback/native",
    ),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl(
      "http://user:password@127.0.0.1:57098/callback/native",
    ),
    null,
  );
  assert.equal(
    normalizeOAuthRedirectUrl(
      "http://127.0.0.1:57098/callback/native#access-token",
    ),
    null,
  );
